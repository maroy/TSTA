...
def create_index_database(path):
    ...
    con.execute("CREATE TABLE tweets(db, id, created, utc_timestamp, screen_name, content)")
...
def save_tweets(path, tweets):
    ...
    sql = "INSERT INTO tweets(db, id, created, utc_timestamp, screen_name, content) VALUES(?, ?, ?, ?, ?, ?)"
    idx_con.executemany(
        sql,
        [(t.db, t.id, t.created, t.utc_timestamp, t.screen_name, t.content) for t in tweets]
    )
    idx_con.commit()
...
def main():
    ...
    for db_file in glob.glob(pattern):
        if not db_file.endswith("idx.db"):
            in_dbs.append(os.path.basename(db_file))

    idx_db_path = os.path.join(folder, "idx.db")
